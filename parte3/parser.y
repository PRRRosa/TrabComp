...
listInit: init| init listInit
	;
init: LIT_INTEGER {fprintf(stderr, "exp=%s\n", $1->text);}| LIT_FLOAT | LIT_TRUE | LIT_FALSE | LIT_CHAR
	;
...
expression '+' expression {$$ = 0}|expression '-' expression |expression '*' expression |expression '/' expression |
 init | funCall |TK_IDENTIFIER {$$ = 0;} | TK_IDENTIFIER '[' arrayIndex ']'
	;
